...
\definition{}\textbf{Markov-tulajdonság} A valószínűségi változók sorozatában
az $i+1.$ változó értéke csak az $i.$ változó értékétől függ, azaz
$P(X_{i+1} = x_{i+1} | X_i = x_i, ..., X_1 = x_1) = P(X_{i+1} = x_{i+1} | X_i = x_i)$.

\definition{}\textbf{Homogén Markov-lánc} Olyan Markov-lánc melyben az átmeneti
valószínűség nem függ az időtől, azaz $P(X_{n+1} = j | X_n = i) = p_{i,j}$.

\definition{}\textbf{Átmeneti valószínűségi mátrix}
A Markov-lánc átmeneti valószínűségi mátrixa ezen $p_{i,j}$ elemekből áll.

\definition{}\textbf{Stacionárius eloszlás} Az $X_i$ változó eloszlása ha i tart
a végtelenbe?
...
\definition{}\textbf{Keveredési idő} A Markov lánc azon valószínűségi változójának
az indexe, melynek az eloszlása a stacionárius eloszlás $\epsilon$ sugarú környezetében
van.

\definition{}\textbf{Hitting time} Az $i \rightarrow j$ csúcsok
